...
async def change_pref(query: CallbackQuery):
    ...
    user = await get_user(query.message.chat.id)
    pref = await ScKeynodes.resolve(f"concept_{_type}_lesson", sc_type.CONST_NODE_CLASS)
    await add_perfs(user, pref)
...
async def change_pref(query: CallbackQuery):
    ...
    user = await get_user(query.message.chat.id)
    charect = await ScKeynodes.resolve(CHARACTERISTICS[id][0], sc_type.CONST_NODE_CLASS)
    await add_charects(user, charect)
