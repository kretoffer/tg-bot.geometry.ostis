    ...
    if len(content) == 1:
        ...
        if content.startswith("http"):
            if content.endswith((".jpg", ".jpeg", ".png")):
                await bot.send_photo(chat_id, photo=content, reply_markup=markup)
            elif content.endswith((".mp4", ".mov", ".avi")):
                await bot.send_video(chat_id, video=content, reply_markup=markup)
            elif content.endswith((".mp3", ".ogg", ".wav")):
                await bot.send_voice(chat_id, voice=content, reply_markup=markup)
            else:
                await bot.send_message(chat_id, content, reply_markup=markup)
        else:
